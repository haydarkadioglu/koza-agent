...
def cmd_update(args: list) -> None:
    ...
    # Determine repo root (two levels up from this file: cli/ -> repo root)
    ...
    # Reinstall package in-place so entry-point and metadata stay in sync
    if pip:
        try:
            result = subprocess.run(
                [pip, "install", "-e", ".", "--quiet"],
...
from cli.cmd_sync import cmd_sync  # noqa: F401  (re-exported for koza_run)
