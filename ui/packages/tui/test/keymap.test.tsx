...
import { getKozaModeStack, KOZA_BASE_MODE, KozaKeymapProvider, registerKozaKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerKozaKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when koza mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getKozaModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getKozaModeStack(keymap).push("autocomplete")
