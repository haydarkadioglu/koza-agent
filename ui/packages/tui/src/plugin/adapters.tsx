import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@koza-ai/plugin/tui"
...
import type { useKozaKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useKozaKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getKozaModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getKozaModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
