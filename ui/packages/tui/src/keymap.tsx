...
export const KOZA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const KOZA_MODE_KEY = "koza.mode"

export const KozaKeymapProvider = KeymapProvider
export const useKozaKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type KozaModeStack = ReturnType<typeof createKozaModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, KozaModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createKozaModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(KOZA_MODE_KEY, KOZA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(KOZA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(KOZA_MODE_KEY, stack.at(-1)?.mode ?? KOZA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? KOZA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(KOZA_MODE_KEY, undefined)
...
export function useKozaModeStack() {
  return getKozaModeStack(useKozaKeymap())
}

export function getKozaModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Koza mode stack is not registered for this keymap")
  return value
}
...
export function registerKozaKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createKozaModeStack(keymap)
