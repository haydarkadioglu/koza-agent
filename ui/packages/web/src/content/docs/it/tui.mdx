---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Koza.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Koza fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Koza avvia la TUI per la directory corrente.
...
Quando usi la TUI di Koza, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Koza. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `koza.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Koza attende dopo il leader key. Default: `2000`.
...
Usa `KOZA_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `koza.default`.
