---
title: Plugin
description: Scrivi plugin per estendere Koza.
---

I plugin ti permettono di estendere Koza agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Koza.
...
- `.koza/plugins/` - plugin a livello progetto
- `~/.config/koza/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/koza/node_modules/`.
...
1. Config globale (`~/.config/koza/koza.json`)
2. Config di progetto (`koza.json`)
3. Directory plugin globale (`~/.config/koza/plugins/`)
4. Directory plugin di progetto (`.koza/plugins/`)
...
Koza esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di koza per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere koza.
...
:::note
Se usi l'app desktop di Koza, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a koza di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a koza:
...
L'helper `tool` crea uno strumento personalizzato che koza può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in koza insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://koza.ai/docs/sdk) per i dettagli.
