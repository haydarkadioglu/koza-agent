...
Puoi aggiungere strumenti esterni a Koza usando il _Model Context Protocol_, o MCP. Koza supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Koza Config](https://koza.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/koza`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Koza gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Koza:
...
Se il server richiede autenticazione, Koza ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `koza mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Koza memorizzera' i token in modo sicuro in `~/.local/share/koza/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Koza insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Koza come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Koza al tuo account Sentry.
