...
Le skill degli agenti permettono a Koza di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Koza cerca in queste posizioni:

- Config di progetto: `.koza/skills/<name>/SKILL.md`
- Config globale: `~/.config/koza/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Koza risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.koza/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.koza/skills/git-release/SKILL.md` cosi':
...
Koza elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `koza.json`:
...
**Per agenti integrati** (in `koza.json`):
