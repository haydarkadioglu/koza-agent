---
title: Server
description: Interagisci con il server koza via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `koza serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client koza.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `koza.local` |
...
Imposta `KOZA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `koza`, oppure imposta `KOZA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `koza serve` sia per `koza web`.
...
Quando esegui `koza` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server koza per interagire con koza in modo programmatico.
:::

Questa architettura permette a koza di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `koza serve` per avviare un server standalone. Se la TUI di koza e gia in esecuzione, `koza serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Koza.
...
Il server koza espone le seguenti API.
