...
With Koza you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Koza uses our own `koza` theme.
...
Koza comes with several built-in themes.
...
- Want Koza to match their terminal's appearance
...
Koza supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/koza/themes/*.json` or `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.koza/themes/*.json`
4. **Current working directory** - Defined in `./.koza/themes/*.json`
