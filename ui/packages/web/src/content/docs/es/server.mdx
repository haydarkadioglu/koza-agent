---
title: Servidor
description: Interactuar con el servidor koza a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `koza serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente koza puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `koza.local` |
...
Configure `KOZA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `koza`, o configure `KOZA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `koza serve` como a `koza web`.
...
Cuando ejecuta `koza`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor koza para interactuar con koza mediante programación.
:::

Esta arquitectura permite que koza admita múltiples clientes y le permite interactuar con koza mediante programación.

Puede ejecutar `koza serve` para iniciar un servidor independiente. Si tienes el
koza TUI ejecutándose, `koza serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Koza [IDE](/docs/ide).
...
El servidor koza expone las siguientes API.
