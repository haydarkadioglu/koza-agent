...
description: Cree herramientas que LLM pueda llamar en koza.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de koza como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.koza/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/koza/tools/`.
...
```ts title=".koza/tools/database.ts" {1}
...
```ts title=".koza/tools/project.ts" {8}
import { tool } from "@koza-ai/plugin"
...
```python title=".koza/tools/add.py"
import sys
...
```ts title=".koza/tools/python-add.ts" {10}
import { tool } from "@koza-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".koza/tools/add.py")
