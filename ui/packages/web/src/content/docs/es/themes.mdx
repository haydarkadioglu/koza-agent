...
Con Koza puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Koza usa nuestro propio tema `koza`.
...
Koza viene con varios temas integrados.
...
- Quiere que Koza coincida con la apariencia de su terminal
...
Koza admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/koza/themes/*.json` o `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.koza/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.koza/themes/*.json`
