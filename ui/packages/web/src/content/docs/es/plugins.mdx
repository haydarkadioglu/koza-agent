---
title: Complementos
description: Escriba sus propios complementos para extender Koza.
---

Los complementos le permiten extender Koza al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Koza.
...
- `.koza/plugins/` - Complementos a nivel de proyecto
- `~/.config/koza/plugins/` - Complementos globales
...
```json title="koza.json"
{
  ...
  "plugin": ["koza-helicone-session", "koza-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/koza/node_modules/`.
...
1. Configuración global (`~/.config/koza/koza.json`)
2. Configuración del proyecto (`koza.json`)
3. Directorio global de complementos (`~/.config/koza/plugins/`)
4. Directorio de complementos del proyecto (`.koza/plugins/`)
...
Koza ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".koza/plugins/my-plugin.ts"
...
```js title=".koza/plugins/example.js"
...
- `client`: Un cliente SDK koza para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar koza.
...
```js title=".koza/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "koza"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Koza, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que koza lea archivos `.env`:

```javascript title=".koza/plugins/env-protection.js"
...
```javascript title=".koza/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a koza:

```ts title=".koza/plugins/custom-tools.ts"
import { type Plugin, tool } from "@koza-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que koza puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para koza junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://koza.ai/docs/sdk) para obtener más detalles.
...
```ts title=".koza/plugins/compaction.ts"
import type { Plugin } from "@koza-ai/plugin"
...
```ts title=".koza/plugins/custom-compaction.ts"
import type { Plugin } from "@koza-ai/plugin"
