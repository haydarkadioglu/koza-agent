...
Las habilidades del agente permiten a Koza descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Koza busca estas ubicaciones:

- Configuración del proyecto: `.koza/skills/<name>/SKILL.md`
- Configuración global: `~/.config/koza/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Koza sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.koza/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.koza/skills/git-release/SKILL.md` así:
...
Koza enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `koza.json`:
...
**Para agentes integrados** (en `koza.json`):
