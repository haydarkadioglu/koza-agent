...
Las herramientas permiten que LLM realice acciones en su código base. Koza viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Koza.
...
:::note
Esta herramienta solo está disponible cuando `KOZA_EXPERIMENTAL_LSP_TOOL=true` (o `KOZA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Koza o cuando la variable de entorno `KOZA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Koza:

```bash
KOZA_ENABLE_EXA=1 koza
```
