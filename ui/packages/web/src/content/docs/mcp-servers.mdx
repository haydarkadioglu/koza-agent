...
You can add external tools to Koza using the _Model Context Protocol_, or MCP. Koza supports both local and remote servers.
...
You can define MCP servers in your [Koza Config](https://koza.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/koza` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Koza automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Koza will:
...
If the server requires authentication, Koza will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `koza mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Koza will store the tokens securely in `~/.local/share/koza/mcp-auth.json`.
...
Your MCPs are available as tools in Koza, alongside built-in tools. So you can manage them through the Koza config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Koza to your Sentry account.
