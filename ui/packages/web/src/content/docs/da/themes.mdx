...
Med Koza kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Koza vårt eget `koza`-tema.
...
Koza kommer med flere innebygde temaer.
...
- Vil at Koza skal matche terminalens utseende
...
Koza støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/koza/themes/*.json` eller `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.koza/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.koza/themes/*.json`
