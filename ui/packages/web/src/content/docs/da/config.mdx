---
title: Konfiguration
description: Ved at bruge Koza JSON konfig.
---

Du kan konfigurere Koza ved hjælp af en JSON-konfigurationsfil.

---

## Format

Koza understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/koza`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/koza/koza.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`KOZA_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`koza.json` i projekt) - projektspecifikke indstillinger
5. **`.koza` mapper** - agent, kommandoer, plugins
6. **Inline config** (`KOZA_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.koza` og `~/.config/koza` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/koza`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Koza-konfiguration i `~/.config/koza/koza.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `koza.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Koza starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `KOZA_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `KOZA_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.koza` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.koza` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`koza.ai/config.json`**](https://koza.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `koza serve` og `koza web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Koza-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `koza.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Koza-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Koza at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Koza-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/koza/agents/` eller `.koza/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Koza falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`koza run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/koza/commands/` eller `.koza/commands/`. [Learn more here](/docs/commands).
...
Koza vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard koza **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Koza med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.koza/plugins/` eller `~/.config/koza/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Koza til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
