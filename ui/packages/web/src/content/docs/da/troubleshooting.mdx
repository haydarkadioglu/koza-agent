...
For at fejlfinde problemer med Koza, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\koza\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `koza --log-level DEBUG`.

---

## Lagring

koza gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/koza/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\koza`
...
Koza Desktop kører en lokal Koza-server (`koza-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Koza`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/koza/koza.jsonc` (eller `~/.config/koza/koza.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/koza/koza.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\koza\koza.jsonc`
...
Koza kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/koza/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\koza\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Koza kan genopbygge den.

1. Afslut Koza Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/koza`
- **Linux**: slet `~/.cache/koza` (eller kør `rm -rf ~/.cache/koza`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\koza`

3. Start Koza Desktop på ny.

---

### Løs problemer med serverforbindelse

Koza Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `koza.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `KOZA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `KOZA_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Koza Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Koza.

---

### Meddelelser vises ikke

Koza Desktop viser kun systemvarsler når:

- varsler er aktiveret for Koza i dine OS-indstillinger, og
...
1. Afslut Koza Desktop.
2. Find og slet disse filer (de findes i Koza Desktop-appens datamappe):

- `koza.settings.dat` (skrivebordsstandardserver URL)
- `koza.global.dat` og `koza.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Koza:
...
For at finde ud af hvilke modeller du har adgang til, kør `koza models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\koza`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. koza installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\koza`

2. Start koza på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge koza til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
koza vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
