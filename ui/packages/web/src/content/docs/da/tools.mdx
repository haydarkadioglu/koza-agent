...
Her er alle de innebygde verktøyene som er tilgængelige i Koza.
...
:::note
Dette verktøyet er bare tilgængelig når `KOZA_EXPERIMENTAL_LSP_TOOL=true` (eller `KOZA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Koza-leverandøren eller når miljøvariabelen `KOZA_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
