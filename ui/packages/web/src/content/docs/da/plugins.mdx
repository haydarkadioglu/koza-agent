---
title: Plugins
description: Skriv dine egne plugins for at udvide Koza.
---

Plugins giver dig mulighed for at udvide Koza ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Kozas standardadfærd.
...
- `.koza/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/koza/node_modules/`.
...
1. Global konfiguration (`~/.config/koza/koza.json`)
2. Projektkonfiguration (`koza.json`)
3. Global plugin-mappe (`~/.config/koza/plugins/`)
4. Projekt plugin bibliotek (`.koza/plugins/`)
...
Koza kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En koza SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide koza.
...
:::note
Hvis du bruger Koza desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå koza i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til koza:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som koza kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for koza sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://koza.ai/docs/sdk) for detaljer.
