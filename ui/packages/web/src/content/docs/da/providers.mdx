---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Koza.
---

import config from "../../../../config.mjs"
export const console = config.console

Koza bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Koza-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/koza/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Koza-konfigurationen.
...
Koza Zen er en liste over modeller leveret af Koza-teamet, som er blevet testet og verificeret til at fungere godt med Koza. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Koza Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg koza og gå til [koza.ai/auth](https://koza.ai/auth).
...
Det fungerer som alle andre udbydere i Koza og er helt valgfrit at bruge.

---

## Koza Go

Koza Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Koza-teamet, som er testet og verificeret til at fungere godt med Koza.

1. Kør kommandoen `/connect` i TUI, vælg `Koza Go`, og gå til [koza.ai/auth](https://koza.ai/zen).
...
Sådan bruger du Amazon Bedrock med Koza:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører koza:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `koza.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Koza understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere koza til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at koza skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem koza-konfigurationen.
...
gemme tokenet i koza auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Koza bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Koza
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`koza.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [koza-gitlab-auth](https://www.npmjs.com/package/koza-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `koza.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Koza:
...
Sådan bruger du Google Vertex AI med Koza:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem koza, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [koza-helicone-session](https://github.com/H2Shami/koza-helicone-session) plugin for automatisk at logge hver Koza-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Koza-samtale opført som en separat session.
...
Du kan konfigurere koza til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere koza til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere koza til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Koza. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/koza) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Koza:
...
3. Kopier API-nøglen til brug i Koza.
...
6. **Vigtigt**: Før du bruger skymodeller i Koza, skal du hente modelinformationen lokalt:
...
Koza Zen er en liste over testede og verificerede modeller leveret af Koza-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Koza Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Koza Zen**.

   ```txt
   /connect
   ```

3. Indtast Koza API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem koza-konfigurationen.
...
5. Du kan også tilpasse dem gennem koza-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Koza:
...
Du kan også tilpasse modeller gennem koza-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med koza. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `koza.json`-filen i projektkataloget:
...
`limit`-felterne lader Koza forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `koza auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek koza-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i koza-konfigurationen.
