...
Agentferdigheter lar Koza oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Koza søker etter disse stedene:

- Prosjektkonfigurasjon: `.koza/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/koza/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Koza opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.koza/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.koza/skills/git-release/SKILL.md` slik:
...
Koza viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `koza.json`:
...
**For innebygde agenter** (i `koza.json`):
