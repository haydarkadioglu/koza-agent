...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Koza posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Koza.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `KOZA_EXPERIMENTAL_LSP_TOOL=true` (lub `KOZA_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Koza lub gdy zmienna środowiskowa `KOZA_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Koza:
