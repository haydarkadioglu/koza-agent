---
title: Serwer
description: Interakcja z serwerem koza za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `koza serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient koza.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `koza.local` |
...
Ustaw `KOZA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `koza` lub ustaw `KOZA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `koza serve`, jak i `koza web`.
...
Kiedy uruchomisz `koza`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera koza do programowej interakcji z koza.
:::

Ta architektura umożliwia obsługę wielu klientów przez koza i programową interakcję z kodem otwartym.

Możesz uruchomić `koza serve`, aby uruchomić samodzielny serwer. Jeśli masz
koza TUI działa, `koza serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki koza [IDE](/docs/ide).
...
Serwer koza udostępnia następujące interfejsy API.
