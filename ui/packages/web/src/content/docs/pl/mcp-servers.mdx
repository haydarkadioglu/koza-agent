...
Możesz dodać dodatkowe informacje do koza za pomocą _Model Context Protocol_, w skrócie MCP. koza obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [koza Config](https://koza.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/koza`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
koza automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, koza:
...
Jeśli serwer wymaga uwierzytelnienia, koza poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `koza mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji koza bezpieczne przechowa tokeny w `~/.local/share/koza/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w koza, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie koza, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć koza z kontem Sentry.
