---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w koza.
---

import config from "../../../../config.mjs"
export const console = config.console

koza używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji koza.
...
w `~/.local/share/koza/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim koza
config.
...
Koza Zen to lista modeli dostarczonych przez zespół koza, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z koza. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Koza Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Koza Zen` i przejdź do [koza.ai/auth](https://koza.ai/zen).
...
Działa jak każdy inny dostawca w koza i jest całkowicie opcjonalny w użyciu.

---

## Koza Go

Koza Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół koza, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z koza.

1. Uruchom polecenie `/connect` w TUI, wybierz `Koza Go` i przejdź do [koza.ai/auth](https://koza.ai/zen).
...
Aby używać Amazon Bedrock z koza:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania koza:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `koza.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w koza nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować koza do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod koza działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji koza.
...
koza używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować koza
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`koza.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [koza-gitlab-auth](https://www.npmjs.com/package/koza-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem koza:
...
Aby używać Google Vertex AI z koza:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania koza.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą koza, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [koza-helicone-session](https://github.com/H2Shami/koza-helicone-session), aby automatycznie rejestrować każdą konwersację koza jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja koza będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować koza tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować koza tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować koza tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem koza. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/koza).
:::
...
Aby korzystać z Ollama Cloud z koza:
...
3. Skopiuj klucz API do użycia w koza.
...
6. **Ważne**: Przed użyciem modeli chmurowych w koza musisz pobrać informacje o modelu lokalnie:
...
Koza Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół koza. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Koza Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Koza Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API koza.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji koza.
...
5. Możesz je także dostosować za pomocą konfiguracji koza. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z koza:
...
Możesz także dostosować modele za pomocą konfiguracji koza. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z koza. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `koza.json` w katalogu projektu:
...
Pola `limit` pozwalają koza zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `koza auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację koza i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji koza.
