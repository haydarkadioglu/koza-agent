---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć koza.
---

Wtyczki umożliwiające wyłączenie koza poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania koza.
...
- `.koza/plugins/` - Wtyczki na poziomie projektu
- `~/.config/koza/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/koza/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/koza/koza.json`)
2. Konfiguracja projektu (`koza.json`)
3. Globalny katalog wtyczek (`~/.config/koza/plugins/`)
4. Katalog wtyczek projektu (`.koza/plugins/`)
...
koza uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z koza do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do koza.
...
:::note
Jeśli korzystasz z aplikacji komputerowej koza, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj koza czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do koza:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod koza. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://koza.ai/docs/sdk).
