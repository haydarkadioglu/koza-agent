...
Dzięki koza możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie koza używa naszego własnego motywu `koza`.
...
koza ma kilka wbudowanych motywów.
...
- Chcą, aby Koza pasował do wyglądu ich terminala
...
koza obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/koza/themes/*.json` lub `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.koza/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.koza/themes/*.json`
