...
Umiejętności agenta pozwalają koza odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
koza przeszukuje te lokalizacje:

- Project config: `.koza/skills/<name>/SKILL.md`
- Global config: `~/.config/koza/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu koza przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.koza/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.koza/skills/git-release/SKILL.md` w ten sposób:
...
koza wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `koza.json`:
...
**Dla agentów wbudowanych** (w `koza.json`):
