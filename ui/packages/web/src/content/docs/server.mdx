---
title: Server
description: Interact with koza server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `koza serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an koza client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `koza.local` |
...
Set `KOZA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `koza`, or set `KOZA_SERVER_USERNAME` to override it. This applies to both `koza serve` and `koza web`.
...
When you run `koza` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the koza server to interact with koza programmatically.
:::

This architecture lets koza support multiple clients and allows you to interact with koza programmatically.

You can run `koza serve` to start a standalone server. If you have the
koza TUI running, `koza serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Koza [IDE](/docs/ide) plugins.
...
The koza server exposes the following APIs.
