...
Wenn Koza Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\koza\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `koza --log-level DEBUG`.

---

## Speicher

koza speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/koza/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\koza`
...
Koza Desktop startet im Hintergrund einen lokalen Koza-Server (`koza-cli`-Sidecar).
...
- Nur macOS: `Koza`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/koza/koza.jsonc` (oder `~/.config/koza/koza.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/koza/koza.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\koza\koza.jsonc`
...
Koza kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/koza/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\koza\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Koza ihn neu aufbauen kann.

1. Beende Koza Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/koza`
- **Linux**: loesche `~/.cache/koza` (oder fuehre aus: `rm -rf ~/.cache/koza`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\koza`

3. Starte Koza Desktop neu.

---

### Server-Verbindungsprobleme beheben

Koza Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `koza.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `KOZA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `KOZA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Koza Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Koza-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Koza Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Koza in den OS-Einstellungen aktiviert sind, und
...
1. Beende Koza Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Koza Desktop):

- `koza.settings.dat` (desktop default server URL)
- `koza.global.dat` und `koza.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Koza hast:
...
3. Stelle sicher, dass du die neueste Version hast: `koza upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `koza models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\koza`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. koza installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\koza`

2. Starte koza neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt koza, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
koza erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
