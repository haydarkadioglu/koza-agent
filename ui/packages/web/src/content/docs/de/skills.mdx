...
Agent Skills erlauben Koza, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Koza sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Koza vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.koza/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.koza/skills/git-release/SKILL.md` zum Beispiel so:
...
Koza listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `koza.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `koza.json`):
