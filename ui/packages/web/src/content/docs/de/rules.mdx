---
title: Regeln
description: Lege eigene Anweisungen fuer koza fest.
---

Du kannst koza ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in koza den Befehl `/init` aus.
...
So kann koza sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht koza sie zu erweitern.
...
koza kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/koza/AGENTS.md` speichern.
Diese gelten in allen koza-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Koza dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/koza/AGENTS.md` existiert)
...
Beim Start sucht koza in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/koza/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `koza.json` oder in `~/.config/koza/koza.json` angeben.
...
koza parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit koza.json

Empfohlen ist das `instructions`-Feld in der `koza.json`:
...
Du kannst koza explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `koza.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
