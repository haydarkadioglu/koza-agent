...
Erstellen Sie `.koza/commands/test.md`:

```md title=".koza/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Koza-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Koza [config](/docs/config):

```json title="koza.jsonc" {4-12}
{
...
- Global: `~/.config/koza/commands/`
- Pro Projekt: `.koza/commands/`

```markdown title="~/.config/koza/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".koza/commands/component.md"
---
...
```md title=".koza/commands/create-file.md"
---
...
```md title=".koza/commands/analyze-coverage.md"
---
...
```md title=".koza/commands/review-changes.md"
---
...
```md title=".koza/commands/review-component.md"
---
...
Koza enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
