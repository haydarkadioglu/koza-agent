---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Koza zu erweitern.
---

Mit Plugins können Sie Koza erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Koza zu ändern.
...
- `.koza/plugins/` – Plugins auf Projektebene
- `~/.config/koza/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/koza/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/koza/koza.json`)
2. Projektkonfiguration (`koza.json`)
3. Globales Plugin-Verzeichnis (`~/.config/koza/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.koza/plugins/`)
...
Koza führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Koza-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Koza erweitern können.
...
:::note
Wenn Sie die Koza-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Koza `.env`-Dateien liest:
...
Plugins können Koza auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Koza aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Koza zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://koza.ai/docs/sdk).
