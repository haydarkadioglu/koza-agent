...
Mit Koza kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Koza das `koza`-Theme.
...
Koza liefert mehrere eingebaute Themes mit.
...
- Koza optisch ans Terminal anpassen wollen
...
Koza unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/koza/themes/*.json` oder `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.koza/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.koza/themes/*.json`
