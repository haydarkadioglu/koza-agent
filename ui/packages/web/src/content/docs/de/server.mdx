---
title: Server
description: Nutze den koza-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `koza serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein koza-Client nutzen kann.
...
Setze `KOZA_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `koza`, kann aber mit `KOZA_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `koza serve` und `koza web`.
...
Wenn du `koza` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den koza-Server, um koza programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `koza serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `koza serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Koza-[IDE](/docs/ide)-Plugins.
...
Der koza-Server stellt folgende APIs bereit.
