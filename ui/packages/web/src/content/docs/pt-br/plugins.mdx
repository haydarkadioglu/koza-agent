---
title: Plugins
description: Escreva seus próprios plugins para estender o koza.
---

Plugins permitem que você estenda o koza conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do koza.
...
- `.koza/plugins/` - Plugins em nível de projeto
- `~/.config/koza/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/koza/node_modules/`.
...
1. Configuração global (`~/.config/koza/koza.json`)
2. Configuração do projeto (`koza.json`)
3. Diretório de plugins global (`~/.config/koza/plugins/`)
4. Diretório de plugins do projeto (`.koza/plugins/`)
...
O koza executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do koza para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o koza.
...
:::note
Se você estiver usando o aplicativo desktop koza, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o koza de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao koza:
...
O helper `tool` cria uma ferramenta personalizada que o koza pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o koza junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://koza.ai/docs/sdk) para detalhes.
