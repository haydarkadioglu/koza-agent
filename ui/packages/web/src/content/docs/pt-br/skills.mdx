...
As habilidades do agente permitem que o koza descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O koza pesquisa nesses locais:

- Configuração do projeto: `.koza/skills/<name>/SKILL.md`
- Configuração global: `~/.config/koza/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o koza sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.koza/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.koza/skills/git-release/SKILL.md` assim:
...
O koza lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `koza.json`:
...
**Para agentes embutidos** (em `koza.json`):
