...
Com o koza, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o koza usa nosso próprio tema `koza`.
...
O koza vem com vários temas embutidos.
...
- Querem que o koza corresponda à aparência do seu terminal
...
O koza suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/koza/themes/*.json` ou `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.koza/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.koza/themes/*.json`
