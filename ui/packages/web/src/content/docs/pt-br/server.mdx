---
title: Servidor
description: Interaja com o servidor koza via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `koza serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente koza pode usar.
...
```bash
koza serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `koza.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
koza serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `KOZA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `koza`, ou defina `KOZA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `koza serve` quanto ao `koza web`.

```bash
KOZA_SERVER_PASSWORD=your-password koza serve
```
...
Quando você executa `koza`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor koza para interagir com o koza programaticamente.
:::

Essa arquitetura permite que o koza suporte múltiplos clientes e permite que você interaja com o koza programaticamente.

Você pode executar `koza serve` para iniciar um servidor autônomo. Se você tiver o TUI do koza em execução, `koza serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do koza [IDE](/docs/ide).
...
O servidor koza expõe as seguintes APIs.
