---
title: サーバー
description: HTTP 経由で Koza サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`koza serve` コマンドは、Koza クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `koza.local` |
...
HTTP 基本認証でサーバーを保護するには、`KOZA_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `koza` になるか、`KOZA_SERVER_USERNAME` を設定してオーバーライドします。これは、`koza serve` と `koza web` の両方に当てはまります。
...
`koza` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Koza サーバーを使用して、プログラムで Koza と対話します。
:::

`koza serve` を実行してスタンドアロンサーバーを起動できます。すでに Koza TUI を実行している場合でも、`koza serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Koza [IDE](/docs/ide) プラグインによって使用されます。
