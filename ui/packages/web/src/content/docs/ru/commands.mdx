...
Создайте `.koza/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию koza или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем koza [config](/docs/config):
...
- Глобальный: `~/.config/koza/commands/`
- Для каждого проекта: `.koza/commands/`
...
koza включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
