---
title: SDK
description: Client JS de type sécurisé pour le serveur koza.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK koza JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler koza par programme.
...
Créez une instance de koza :

```javascript
import { createKoza } from "@koza-ai/sdk"

const { client } = await createKoza()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `koza.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createKoza } from "@koza-ai/sdk"

const koza = await createKoza({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${koza.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de koza, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@koza-ai/sdk"
```
