---
title: Plugins
description: Écrivez vos propres plugins pour étendre Koza.
---

Les plugins vous permettent d'étendre Koza en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Koza.
...
- `.koza/plugins/` - Plugins au niveau du projet
- `~/.config/koza/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/koza/node_modules/`.
...
1. Configuration globale (`~/.config/koza/koza.json`)
2. Configuration du projet (`koza.json`)
3. Répertoire global des plugins (`~/.config/koza/plugins/`)
4. Répertoire des plugins du projet (`.koza/plugins/`)
...
Koza exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK koza pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre koza.
...
:::note
Si vous utilisez l'application de bureau Koza, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez koza de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à koza :
...
L'assistant `tool` crée un outil personnalisé que koza peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour koza aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://koza.ai/docs/sdk) pour plus de détails.
