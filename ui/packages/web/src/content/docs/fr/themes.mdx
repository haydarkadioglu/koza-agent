...
Avec Koza, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Koza utilise notre propre thème `koza`.
...
Koza est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Koza corresponde à l'apparence de leur terminal
...
Koza prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/koza/themes/*.json` ou `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.koza/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.koza/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/koza/themes
vim ~/.config/koza/themes/my-theme.json
```
...
{
  "$schema": "https://koza.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
