---
title: Serveur
description: Interagissez avec le serveur koza via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `koza serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client koza peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `koza.local` |
...
Définissez `KOZA_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `koza`, ou définissez `KOZA_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `koza serve` et à `koza web`.
...
Lorsque vous exécutez `koza`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur koza pour interagir avec koza par programmation.
:::

Cette architecture permet à koza de prendre en charge plusieurs clients et vous permet d'interagir avec koza par programme.

Vous pouvez exécuter `koza serve` pour démarrer un serveur autonome. Si vous avez le
koza TUI en cours d'exécution, `koza serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Koza [IDE](/docs/ide).
...
Le serveur koza expose les API suivantes.
