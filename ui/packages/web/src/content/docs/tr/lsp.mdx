...
description: Koza, LSP sunucularınızla bütünleşir.
---

Koza, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Koza, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`KOZA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve koza bir dosyayı açtığında:
...
LSP sunucularını, koza yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
