---
title: Eklentiler
description: Koza'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Koza'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Koza'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.koza/plugins/` - Proje düzeyinde eklentiler
- `~/.config/koza/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/koza/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/koza/koza.json`)
2. Project config (`koza.json`)
3. Global eklenti dizini (`~/.config/koza/plugins/`)
4. Proje eklenti dizini (`.koza/plugins/`)
...
Koza bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Koza SDK istemcisi.
...
Koza'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Koza masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Koza'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Koza'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Koza'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Koza'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://koza.ai/docs/sdk) bakın.
