...
koza ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak koza kendi `koza` temasını kullanır.
...
koza birden fazla yerleşik temayla gelir.
...
- koza'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
koza, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/koza/themes/*.json` veya `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.koza/themes/*.json`
4. **Geçerli çalışma dizini** - `./.koza/themes/*.json`
