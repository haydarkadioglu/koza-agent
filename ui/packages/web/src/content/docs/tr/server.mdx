---
title: Sunucu
description: koza sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`koza serve` komutu, koza istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `koza.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `KOZA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `koza` değeridir; değiştirmek isterseniz `KOZA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `koza serve` hem de `koza web` için geçerlidir.
...
`koza` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
koza ile programatik etkileşim için koza sunucusunu kullanın.
:::

Bu mimari, koza'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`koza serve` ile bağımsız bir sunucu başlatabilirsiniz. koza TUI açıksa, `koza serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum koza [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
koza sunucusu aşağıdaki API'leri sunar.
