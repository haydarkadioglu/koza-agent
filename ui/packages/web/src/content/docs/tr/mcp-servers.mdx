...
_Model Bağlam Protokolü_ veya MCP kullanarak koza'a harici araçlar ekleyebilirsiniz. koza hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [koza Config](https://koza.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/koza` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
koza, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde koza şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa koza, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `koza mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından koza, belirteçleri `~/.local/share/koza/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra koza'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi koza yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve koza'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
