...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. koza yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
koza içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `KOZA_EXPERIMENTAL_LSP_TOOL=true` (veya `KOZA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca koza provider kullanılırken veya `KOZA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

koza başlatırken etkinleştirmek için:
