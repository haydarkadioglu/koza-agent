...
Koza Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Koza'daki diğer sağlayıcılar gibi çalışır. Koza Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Koza'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Koza Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Koza Go, Koza'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Koza Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Koza Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Koza Go'ya abone olabilir.
:::
...
Koza Go aşağıdaki limitleri içerir:
...
Koza yapılandırmanızdaki [model id](/docs/config/#models) formatı `koza-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `koza-go/kimi-k3` kullanmalısınız.
...
Koza Go'yu şu amaçlarla oluşturduk:
...
4. Koza ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
