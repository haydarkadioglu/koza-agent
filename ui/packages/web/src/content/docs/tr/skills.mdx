...
Ajan becerileri, koza'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
koza su konumlari tarar:

- Proje konfigurasyonu: `.koza/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/koza/skills/<name>/SKILL.md`
...
Proje yerel yollarinda koza, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.koza/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.koza/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
koza, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`koza.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`koza.json` icinde):
