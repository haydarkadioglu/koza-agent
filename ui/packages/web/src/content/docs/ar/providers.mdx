---
title: المزوّدون
description: استخدام أي موفّر LLM في Koza.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Koza ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Koza.
...
في `~/.local/share/koza/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Koza.
...
Koza Zen هي قائمة نماذج يوفّرها فريق Koza وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Koza. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Koza Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر koza، ثم انتقل إلى [koza.ai/auth](https://koza.ai/auth).
...
يعمل مثل أي مزوّد آخر في Koza واستخدامه اختياري بالكامل.

---

## Koza Go

Koza Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Koza، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Koza.

1. شغّل الأمر `/connect` في TUI، واختر `Koza Go`، ثم انتقل إلى [koza.ai/auth](https://koza.ai/zen).
...
لاستخدام Amazon Bedrock مع Koza:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل koza:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `koza.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Koza غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين koza لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل koza بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات koza.
...
تخزين الرمز في مساحة تخزين مصادقة koza.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Koza نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Koza
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`koza.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [koza-gitlab-auth](https://www.npmjs.com/package/koza-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع koza:
...
لاستخدام Google Vertex AI مع Koza:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر koza، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [koza-helicone-session](https://github.com/H2Shami/koza-helicone-session) لتسجيل كل محادثة Koza تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Koza مدرجة كجلسة مستقلة.
...
يمكنك ضبط koza لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط koza لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط koza لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Koza. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/koza) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Koza.
...
6. **مهم**: قبل استخدام نماذج السحابة في Koza، يجب سحب معلومات النموذج محليا:
...
Koza Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Koza. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Koza Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Koza Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Koza.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات koza.
...
5. يمكنك أيضا تخصيصها عبر إعدادات koza. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Koza:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات koza. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع koza. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `koza.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Koza بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `koza auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات koza وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات koza.
