---
title: الخادم
description: تفاعل مع خادم koza عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `koza serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل koza استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `koza.local` |
...
عيّن `KOZA_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `koza`، أو عيّن `KOZA_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `koza serve` و `koza web`.
...
عند تشغيل `koza` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم koza للتفاعل مع koza برمجيا.
:::

تتيح هذه البنية لـ koza دعم عدة عملاء وتمكّنك من التفاعل مع koza برمجيا.

يمكنك تشغيل `koza serve` لبدء خادم مستقل. إذا كانت واجهة koza في terminal (TUI)
قيد التشغيل، فسيبدأ `koza serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Koza لـ [IDE](/docs/ide).
...
يعرض خادم koza واجهات API التالية.
