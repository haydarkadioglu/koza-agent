...
يمكنك إضافة أدوات خارجية إلى Koza باستخدام _Model Context Protocol_، أو MCP. يدعم Koza خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Koza](https://koza.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/koza` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Koza تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Koza بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Koza المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `koza mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Koza الرموز بشكل آمن في `~/.local/share/koza/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Koza إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Koza مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Koza بحساب Sentry الخاص بك.
