...
مع Koza يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Koza سمتنا `koza`.
...
- يريدون أن يطابق Koza مظهر terminal لديهم
...
يدعم Koza نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/koza/themes/*.json` أو `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.koza/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.koza/themes/*.json`
