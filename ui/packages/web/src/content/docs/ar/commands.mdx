...
أنشئ الملف `.koza/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Koza أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Koza:
...
- على مستوى النظام: `~/.config/koza/commands/`
- لكل مشروع: `.koza/commands/`
...
يتضمن koza عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
