---
title: Konfiguracija
description: Korištenje Koza JSON konfiguracije.
---

Možete konfigurirati Koza koristeći JSON konfiguracijski fajl.

---

## Format

Koza podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/koza`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/koza/koza.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`KOZA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`koza.json` u projektu) - postavke specifične za projekat
5. **`.koza` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`KOZA_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.koza` i `~/.config/koza` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/koza` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Koza konfiguraciju u `~/.config/koza/koza.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/koza/tui.json`.
...
Dodajte `koza.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Koza pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `KOZA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `KOZA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.koza` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.koza`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`koza.ai/config.json`**](https://koza.ai/config.json).

TUI konfiguracija koristi [**`koza.ai/tui.json`**](https://koza.ai/tui.json).
...
Koristite `KOZA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `koza.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `koza serve` i `koza web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Koza server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `koza.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Koza konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Koza pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/koza/agents/` ili `.koza/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Koza će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`koza run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/koza/commands/` ili `.koza/commands/`. [Saznajte više ovdje](/docs/commands).
...
Koza će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Koza **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Koza sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.koza/plugins/` ili `~/.config/koza/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Koza da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
