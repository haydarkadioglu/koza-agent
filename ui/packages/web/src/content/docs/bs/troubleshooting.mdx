...
Da biste otklonili probleme s Koza, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\koza\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `koza --log-level DEBUG`.

---

## Pohrana

Koza pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/koza/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\koza`
...
Koza Desktop pokreće lokalni Koza server (`koza-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Koza` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/koza/koza.jsonc` (ili `~/.config/koza/koza.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/koza/koza.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\koza\koza.jsonc`
...
Koza također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/koza/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\koza\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Koza mogao ponovo izgraditi.

1. Potpuno zatvorite Koza Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/koza`
- **Linux**: obrišite `~/.cache/koza` (ili pokrenite `rm -rf ~/.cache/koza`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\koza`

3. Ponovo pokrenite Koza Desktop.

---

### Rješavanje problema sa vezom na serveru

Koza Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `koza.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `KOZA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `KOZA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Koza Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Koza karakteristikama.

---

### Obavještenja se ne prikazuju

Koza Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Koza u postavkama vašeg OS-a, i
...
1. Zatvorite Koza Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Koza Desktop aplikacije):

- `koza.settings.dat` (URL zadanog servera za desktop)
- `koza.global.dat` i `koza.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `koza upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `koza models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\koza`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Koza dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\koza`

2. Ponovo pokrenite Koza da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Koza da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Koza će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
