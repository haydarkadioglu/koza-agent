---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Koza.
---

import config from "../../../../config.mjs"
export const console = config.console

Koza koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Koza konfiguraciji.
...
u `~/.local/share/koza/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Koza
config.
...
Koza Zen je lista modela koje je obezbedio Koza tim koji su bili
testirano i potvrđeno da dobro radi sa Koza. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Koza Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite koza i idite na [koza.ai/auth](https://koza.ai/auth).
...
Radi kao i svaki drugi provajder u Koza i potpuno je opcionalan za korištenje.

---

## Koza Go

Koza Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Koza tim i koji su testirani i verificirani da dobro rade s Koza-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Koza Go` i idite na [koza.ai/auth](https://koza.ai/zen).
...
Da biste koristili Amazon Bedrock s Koza:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete koza:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `koza.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Koza.
:::
...
Možete konfigurirati koza za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi koza ispravno radio.
   :::
...
Također možete dodati modele kroz svoju koza konfiguraciju.
...
da pohrani token u koza auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Koza koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Koza
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`koza.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [koza-gitlab-auth](https://www.npmjs.com/package/koza-gitlab-auth).

##### Konfiguracija

Prilagodite putem `koza.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s koza:
...
Za korištenje Google Vertex AI s Koza:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće koza.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem kozaa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [koza-helicone-session](https://github.com/H2Shami/koza-helicone-session) da automatski prijavite svaki Koza razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Koza razgovor naveden kao zasebna sesija.
...
Možete konfigurirati koza za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati koza za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati koza za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Koza. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/koza) za detalje.
:::
...
Da biste koristili Ollama Cloud s Koza:
...
3. Kopirajte API ključ za korištenje u Koza.
...
6. **Važno**: Prije upotrebe modela oblaka u Koza, morate lokalno povući informacije o modelu:
...
Koza Zen je lista testiranih i verifikovanih modela koju je obezbedio Koza tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Koza Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Koza Zen**.

```txt
   /connect
```

3. Unesite svoj Koza API ključ.
...
Također možete dodati dodatne modele putem vaše koza konfiguracije.
...
5. Također ih možete prilagoditi putem vaše koza konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Kozaom:
...
Također možete prilagoditi modele kroz svoju koza konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s koza-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `koza.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Koza da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `koza auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Koza konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj koza konfiguraciji.
