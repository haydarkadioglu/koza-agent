...
Možete dodati vanjske alate u Koza koristeći _Model Context Protocol_, ili MCP. Koza podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Koza Config](https://koza.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/koza`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Koza automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Koza će:
...
Ako server zahtijeva autentifikaciju, Koza će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `koza mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Koza sigurno čuva tokene u `~/.local/share/koza/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Koza, zajedno s ugrađenim alatima. Možete njima upravljati kroz Koza konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Koza sa vašim Sentry nalogom.
