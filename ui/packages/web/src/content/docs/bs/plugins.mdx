---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Koza.
---

Dodaci vam omogućavaju da proširite Koza spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Koza.
...
- `.koza/plugins/` - Dodaci na nivou projekta
- `~/.config/koza/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/koza/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/koza/koza.json`)
2. Konfiguracija projekta (`koza.json`)
3. Globalni direktorij dodataka (`~/.config/koza/plugins/`)
4. Direktorij dodataka projekta (`.koza/plugins/`)
...
Koza pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Koza SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Koza.
...
Spriječite koza da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u koza:
...
Pomoćnik `tool` kreira prilagođeni alat koji koza može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za koza zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://koza.ai/docs/sdk) za detalje.
