...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Koza dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Koza.
...
:::note
Ovaj alat je dostupan samo kada je `KOZA_EXPERIMENTAL_LSP_TOOL=true` (ili `KOZA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Koza provajdera ili kada je varijabla `KOZA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Koza:
