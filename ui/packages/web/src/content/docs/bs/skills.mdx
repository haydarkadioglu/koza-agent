...
Agent skills omogucavaju Koza da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Koza pretrazuje ove lokacije:

- Konfiguracija projekta: `.koza/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/koza/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Koza ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.koza/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/koza/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.koza/skills/git-release/SKILL.md` ovako:
...
Koza navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `koza.json`:
...
**Za ugradene agente** (u `koza.json`):
