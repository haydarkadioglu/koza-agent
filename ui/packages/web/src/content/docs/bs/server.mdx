---
title: Server
description: Komunicirajte s koza serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `koza serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji koza klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `koza.local` |
...
Postavite `KOZA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `koza`, ili postavite `KOZA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `koza serve` i za `koza web`.
...
Kada pokrenete `koza`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite koza server za programsku interakciju sa koza-om.
:::

Ova arhitektura omogucava koza podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `koza serve` da startate standalone server. Ako je koza TUI vec pokrenut, `koza serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Koza [IDE](/docs/ide) pluginovi.
...
koza server izlaže sljedece API-je.
