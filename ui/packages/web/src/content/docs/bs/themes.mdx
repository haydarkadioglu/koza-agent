...
U Koza mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Koza koristi nasu `koza` temu.
...
Koza dolazi sa vise ugradenih tema.
...
- Zele da Koza odgovara izgledu njihovog terminala
...
Koza podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/koza/themes/*.json` ili `$XDG_CONFIG_HOME/koza/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.koza/themes/*.json`
4. **Trenutni radni direktorij** - `./.koza/themes/*.json`
