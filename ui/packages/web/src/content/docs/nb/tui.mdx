---
title: TUI
description: Bruke koza-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Koza gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Koza starter TUI for gjeldende katalog.
...
Når du bruker Koza TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Koza. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `koza.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Koza venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `koza.default`.
