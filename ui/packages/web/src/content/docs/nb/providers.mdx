---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Koza.
---

import config from "../../../../config.mjs"
export const console = config.console

Koza bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Koza-konfigurasjonen.
...
i `~/.local/share/koza/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Koza
konfig.
...
Koza Zen er en liste over modeller levert av Koza-teamet som har vært
testet og verifisert for å fungere godt med Koza. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Koza Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Koza Zen` og gå til [koza.ai/auth](https://koza.ai/zen).
...
Det fungerer som alle andre leverandører i Koza og er helt valgfritt å bruke.

---

## Koza Go

Koza Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Koza-teamet som har vært
testet og verifisert for å fungere godt med Koza.

1. Kjør kommandoen `/connect` i TUI, velg `Koza Go`, og gå til [koza.ai/auth](https://koza.ai/zen).
...
Slik bruker du Amazon Bedrock med Koza:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Koza:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `koza.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i koza støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere koza til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Koza skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Koza-konfigurasjonen.
...
for å lagre token i Koza auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Koza bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Koza
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`koza.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [koza-gitlab-auth](https://www.npmjs.com/package/koza-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `koza.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Koza:
...
Slik bruker du Google Vertex AI med Koza:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Koza, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [koza-helicone-session](https://github.com/H2Shami/koza-helicone-session) plugin for automatisk å logge hver Koza-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Koza-samtale oppført som en separat økt.
...
Du kan konfigurere Koza for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Koza for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Koza for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Koza. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/koza) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Koza:
...
3. Kopier API-nøkkelen for bruk i Koza.
...
6. **Viktig**: Før du bruker skymodeller i Koza, må du hente modellinformasjonen lokalt:
...
Koza Zen er en liste over testede og verifiserte modeller levert av Koza-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Koza Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Koza Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Koza API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Koza-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Koza-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Koza:
...
Du kan også tilpasse modeller gjennom Koza-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Koza. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `koza.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Koza forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `koza auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Koza-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Koza-konfigurasjonen.
