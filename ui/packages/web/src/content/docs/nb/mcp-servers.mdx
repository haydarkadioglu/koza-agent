...
Du kan legge til eksterne verktøy i Koza ved å bruke _Model Context Protocol_ eller MCP. Koza støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Koza Config](https://koza.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="koza.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/koza`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Koza håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Koza:
...
Hvis serveren krever autentisering, vil Koza be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `koza mcp auth <server-name>`.
...
```bash
koza mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Koza lagre tokens sikkert i `~/.local/share/koza/mcp-auth.json`.
...
koza mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Koza, sammen med innebygde verktøy. Så du kan administrere dem gjennom Koza-konfigurasjonen som alle andre verktøy.
...
```bash
koza mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Koza til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Koza-samtale oppført som en separat økt.
