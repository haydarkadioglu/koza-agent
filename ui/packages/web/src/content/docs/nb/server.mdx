---
title: Tjener
description: Samhandle med Koza-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `koza serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Koza-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `koza.local` |
...
Sett `KOZA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `koza` som standard, eller sett `KOZA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `koza serve` og `koza web`.
...
Når du kjører `koza` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Koza-serveren til å samhandle med Koza programmatisk.
:::

Denne arkitekturen lar Koza støtte flere klienter og lar deg samhandle med Koza programmatisk.

Du kan kjøre `koza serve` for å starte en frittstående server. Hvis du har
Koza TUI kjører, vil `koza serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Koza [IDE](/docs/ide) plugins.
...
koza-serveren viser følgende APIer.
