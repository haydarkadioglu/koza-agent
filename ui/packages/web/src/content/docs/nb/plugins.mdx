...
description: Skriv dine egne programtillegg for å utvide Koza.
---

Plugins lar deg utvide Koza ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Koza.
...
- `.koza/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/koza/node_modules/`.
...
1. Global konfigurasjon (`~/.config/koza/koza.json`)
2. Prosjektkonfigurasjon (`koza.json`)
3. Global plugin-katalog (`~/.config/koza/plugins/`)
4. Prosjektpluginkatalog (`.koza/plugins/`)
...
Koza kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Koza SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Koza.
...
:::note
Hvis du bruker Koza-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Koza fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Koza:
...
`tool`-hjelperen lager et tilpasset verktøy som Koza kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Koza sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://koza.ai/docs/sdk) for detaljer.
