...
import "./styles.css"
import { Splash } from "@koza-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.KOZA_CHANNEL === "prod" &&
...
const deepLinkEvent = "koza:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__KOZA__ ??= {}
  const pending = window.__KOZA__.deepLinks ?? []
  window.__KOZA__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `koza.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://koza.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("koza.global.dat").getItem("language")
