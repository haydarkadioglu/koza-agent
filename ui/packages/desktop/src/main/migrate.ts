...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.koza.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `koza.settings.dat` is special: it maps to the `koza.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"koza.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // koza.settings.dat → the electron settings store ("koza.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "koza.global.dat").
  const storeName = filename === "koza.settings.dat" ? "koza.settings" : filename
