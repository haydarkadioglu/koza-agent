...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@koza-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@koza-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@koza-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@koza-ai/tui/context/theme"
import { Global } from "@koza-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@koza-ai/core/util/flock"
import { Flag } from "@koza-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@koza-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@koza-ai/core/v1/config/plugin"
import { createCommandShim } from "@koza-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@koza-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".koza"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".koza", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".koza", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.KOZA_PURE ? [] : pluginOrigins
    if (Flag.KOZA_PURE && pluginOrigins.length) {
    }
