...
export const DEPRECATED_PLUGIN_PACKAGES = ["koza-openai-codex-auth", "koza-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, kozaVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(kozaVersion) || semver.major(kozaVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.koza
  if (typeof range !== "string") return
  if (!semver.satisfies(kozaVersion, range)) {
    throw new Error(`Plugin requires koza ${range} but running ${kozaVersion}`)
  }
}
