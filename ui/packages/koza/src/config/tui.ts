...
import { isRecord } from "@koza-ai/tui/util/record"
...
import { TuiKeybind } from "@koza-ai/tui/config/keybind"
...
import { TuiConfig } from "@koza-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@koza/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old koza.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.koza`
  // folders between cwd and home, and KOZA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.KOZA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit KOZA_TUI_CONFIG override, if set.
  if (Flag.KOZA_TUI_CONFIG) {
    const configFile = Flag.KOZA_TUI_CONFIG
  ...
  // 4. `.koza` directories (and KOZA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".koza") || dir === Flag.KOZA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".koza") && dir !== Flag.KOZA_CONFIG_DIR) continue
