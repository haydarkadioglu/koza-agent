...
const TUI_SCHEMA_URL = "https://koza.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from koza.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const koza = await kozaFiles(input)
  for (const file of koza) {
...
async function kozaFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "koza"),
    ...(await Filesystem.findUp(["koza.json", "koza.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "koza"))
  }
  if (Flag.KOZA_CONFIG) files.push(Flag.KOZA_CONFIG)
