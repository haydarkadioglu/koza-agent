...
import type { LLMClientShape } from "@koza-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("koza"))
    return { type: "unsupported", reason: "provider is not openai, koza, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @koza-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains koza-owned. The native runtime only adapts
      // the @koza-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
