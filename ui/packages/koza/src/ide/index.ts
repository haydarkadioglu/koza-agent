...
import { IdeEvent } from "@koza-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["KOZA_CALLER"] === "vscode" || process.env["KOZA_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.koza"], {
    nothrow: true,
  })
