...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for koza serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(KozaHttpApi)
...
export const PublicApi = KozaHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "koza",
    version: "1.0.0",
    description: "koza api",
