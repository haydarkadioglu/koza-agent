...
import type { AssistantMessage, Message, KozaClient, SessionMessageResponse } from "@koza-ai/sdk/v2"
...
export const AuthMethodID = "koza-login"
...
export class Service extends Context.Service<Service, Interface>()("@koza/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `koza auth login` in the terminal",
      name: "Login with koza",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: KozaClient) {
...
function makeUsageService(sdk: KozaClient) {
...
async function loadDirectorySnapshot(sdk: KozaClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, koza provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const kozaProvider = providers[ProviderV2.ID.make("koza")]
  const kozaModel = kozaProvider ? Provider.sort(Object.values(kozaProvider.models))[0] : undefined
  if (kozaProvider && kozaModel) return { providerID: kozaProvider.id, modelID: kozaModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Koza service failure", service })
}
