// Shared type vocabulary for the direct interactive mode (`koza --mini`).
//
...
import type { KozaClient, PermissionRequest, QuestionRequest, ToolPart } from "@koza-ai/sdk/v2"
import type { TuiConfig } from "@koza-ai/tui/config"
...
type PromptModel = Parameters<KozaClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<KozaClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<KozaClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<KozaClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<KozaClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<KozaClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<KozaClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<KozaClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<KozaClient["question"]["reject"]>[0]
