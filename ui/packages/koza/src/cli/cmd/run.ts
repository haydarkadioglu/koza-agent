import type { PermissionV1 } from "@koza-ai/core/v1/permission"
import { FSUtil } from "@koza-ai/core/fs-util"
// CLI entry point for `koza run` and `koza --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`koza --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`koza --mini --attach`): connects to a running
//      koza server and runs interactive mode against it.
//
...
import { createKozaClient, type KozaClient, type ToolPart } from "@koza-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<KozaClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run koza with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running koza server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to KOZA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to KOZA_SERVER_USERNAME or 'koza')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: KozaClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: KozaClient, sessionID: string) {
      ...
      async function current(sdk: KozaClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: KozaClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: KozaClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: KozaClient) {
        ...
        async function loop(client: KozaClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createKozaClient({
        baseUrl: "http://koza.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
