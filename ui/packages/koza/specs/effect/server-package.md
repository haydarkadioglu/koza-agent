...
Practical reference for a future `packages/server` split after the koza
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/koza`.
...
Until enough shared service code lives outside `packages/koza`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/koza`

It should not import `packages/koza` services while `packages/koza`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on koza-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/koza` implementation details.
