...
- Author package entrypoint is `@koza-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/koza-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@koza-ai/plugin/tui`.
- `@koza-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@koza-ai/plugin/tui"
...
- The value is a semver range checked against the running Koza version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.koza` is absent, no check is performed (backward compatible).
...
- `koza plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `koza plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.koza` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.koza`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.koza`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.koza` dir or `KOZA_CONFIG_DIR`, Koza installs `@koza-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@koza-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Koza's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Koza registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Koza pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@koza-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"koza"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `koza.default`.
...
- Local plugins persist installed themes under the local `.koza/themes` area near the plugin config source.
...
- `--pure` / `KOZA_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.koza/plugins/tui-smoke.tsx`
- Local vim plugin: `.koza/plugins/tui-vim.tsx`
- Local smoke config: `.koza/tui.json`
- Local smoke theme: `.koza/plugins/smoke-theme.json`
