...
Delete `packages/koza/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/koza/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/koza/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/koza`
- TUI plugin loader tests in `packages/koza` if runtime plugin API wiring changed
