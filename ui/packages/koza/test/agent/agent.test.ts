...
it.instance("plan agent denies edits except .koza/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".koza/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".koza", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.KOZA_TEST_HOME
      process.env.KOZA_TEST_HOME = test.directory
