// Subprocess test harness for the koza CLI. Spawns the real binary against
...
//
// Configuration flows through koza's built-in test affordances:
//   - KOZA_CONFIG_CONTENT      : provider config inline, no files to find
//   - KOZA_TEST_HOME           : pins os.homedir() → tmpdir
//   - KOZA_DISABLE_PROJECT_CONFIG : skip walking up for koza.json
//   - KOZA_PURE                : skip external plugin discovery + install
//   - KOZA_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `koza.run` is fully wired. The shape supports adding more
// builders (`koza.serve(opts)`, `koza.acp(opts)`, `koza.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of KozaCli.
...
import { AppProcess } from "@koza-ai/core/process"
...
const kozaRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(kozaRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    KOZA_CONFIG_CONTENT: configJson,
    KOZA_DISABLE_PROJECT_CONFIG: "1",
    KOZA_PURE: "1",
    KOZA_DISABLE_AUTOUPDATE: "1",
    KOZA_DISABLE_AUTOCOMPACT: "1",
    KOZA_DISABLE_MODELS_FETCH: "1",
    KOZA_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `koza run`. New flags should
...
// `koza serve` is a long-lived process — it never exits on its own.
...
// `koza acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type KozaCli = {
  ...
  // Spawn `koza serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `koza acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("koza.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          KOZA_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("koza.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("koza.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "koza server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `koza serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("koza.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const koza: KozaCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, koza })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `koza.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "koza") {
...
// every call site. Use this for any test that needs the koza CLI fixture.
//
...
// resources (e.g. `koza.serve`) without an extra `Effect.scoped` wrapper —
