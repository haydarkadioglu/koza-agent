...
describe("koza read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // KOZA_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["mcp", "list"])
        koza.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["providers", "list"])
        koza.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["models"])
        koza.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["agent", "list"])
        koza.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh KOZA_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["session", "list"])
        koza.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["stats"])
        koza.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under KOZA_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ koza }) =>
      Effect.gen(function* () {
        const r = yield* koza.spawn(["db", "path"])
        koza.expectExit(r, 0, "db path")
