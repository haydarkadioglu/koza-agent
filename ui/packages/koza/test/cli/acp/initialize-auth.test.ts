...
describe("koza acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ koza }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ koza }))
        ...
        expect(initialized.agentInfo?.name).toBe("Koza")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ koza })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("koza-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "koza-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.KOZA_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("koza-login")
