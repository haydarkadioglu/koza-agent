...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ koza }) =>
    Effect.gen(function* () {
      const result = yield* koza.spawn(["--replay-limit", "10"])

      koza.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ koza }) =>
    Effect.gen(function* () {
      const result = yield* koza.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ koza }) =>
    Effect.gen(function* () {
      const result = yield* koza.spawn(["--mini", "--port", "4096"])
