...
`,
  ...
  const pure = process.env.KOZA_PURE
  const meta = process.env.KOZA_PLUGIN_META_FILE
  process.env.KOZA_PURE = "1"
  process.env.KOZA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.KOZA_PURE
    } else {
      process.env.KOZA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.KOZA_PLUGIN_META_FILE
    } else {
      process.env.KOZA_PLUGIN_META_FILE = meta
    }
  }
})
