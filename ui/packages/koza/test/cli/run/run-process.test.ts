// Subprocess integration tests for `koza run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `koza.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `KOZA_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("koza run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("say hi")
        koza.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* koza.run("think", { extraArgs: ["--thinking"] })
        koza.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* koza.run("think again")
        koza.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("say hi", { format: "json" })
        koza.expectExit(result, 0)

        const events = koza.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ koza }) =>
      Effect.gen(function* () {
        const result = yield* koza.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        ...
        const result = yield* koza.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, koza }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* koza.run("request permission", { permission: { bash: "ask" } })
        koza.expectExit(denied, 0)
        ...
        const allowed = yield* koza.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        koza.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* koza.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        koza.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, koza }) =>
      Effect.gen(function* () {
        ...
        const server = yield* koza.serve()

        const result = yield* koza.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, koza }) =>
      Effect.gen(function* () {
        const result = yield* koza.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, koza }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* koza.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
