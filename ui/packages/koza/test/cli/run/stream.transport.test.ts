...
import { KozaClient, type GlobalEvent } from "@koza-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<KozaClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<KozaClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<KozaClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<KozaClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<KozaClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://koza.test"),
...
function sdk(
  input: {
    ...
    subscribe?: KozaClient["event"]["subscribe"]
    globalEvent?: KozaClient["global"]["event"]
    promptAsync?: KozaClient["session"]["promptAsync"]
    status?: KozaClient["session"]["status"]
    messages?: KozaClient["session"]["messages"]
    children?: KozaClient["session"]["children"]
    permissions?: KozaClient["permission"]["list"]
    questions?: KozaClient["question"]["list"]
  } = {},
) {
  const client = new KozaClient()

  const subscribe: KozaClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: KozaClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: KozaClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: KozaClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: KozaClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: KozaClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: KozaClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: KozaClient["question"]["list"] = input.questions ?? (() => ok([]))
