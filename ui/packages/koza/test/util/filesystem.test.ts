...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "koza.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "koza.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "koza.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "koza.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["koza.json", "koza.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "koza.json"),
        path.join(project, "koza.jsonc"),
      ])
    })
  })
