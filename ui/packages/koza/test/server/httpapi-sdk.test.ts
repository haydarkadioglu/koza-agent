...
import { createKozaClient } from "@koza-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createKozaClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.KOZA_SERVER_PASSWORD = input?.password
      Flag.KOZA_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".koza", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-koza-directory")).toBe(false)
        expect(request!.headers.has("x-koza-workspace")).toBe(false)
      }),
    ),
  )
