...
import { SessionV1 } from "@koza-ai/core/v1/session"
import { ModelsDev } from "@koza-ai/core/models-dev"
import { HttpRecorder } from "@koza-ai/http-recorder"
import { HttpRecorderInternal } from "@koza-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@koza-ai/llm"
import { RequestExecutor } from "@koza-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.koza.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.KOZA_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["koza", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KOZA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("KOZA_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["koza", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "koza-proxy",
    name: "Koza proxy",
    providerID: ProviderV2.ID.koza,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["koza", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.KOZA_RECORD_CONSOLE_TOKEN && process.env.KOZA_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.koza,
        name: "Koza Zen",
        env: ["KOZA_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.KOZA_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.KOZA_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.KOZA_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["koza", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KOZA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("KOZA_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("KOZA_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set KOZA_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the koza LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "koza.json"),
      JSON.stringify({ $schema: "https://koza.ai/config.json", ...scenario.config(model) }),
    ),
  )
