...
import { EventSequenceTable } from "@koza-ai/core/event/sql"
...
import { WorkspaceV2 } from "@koza-ai/core/workspace"
import { WorkspaceTable } from "@koza-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@koza-ai/core/ripgrep"
import { AppNodeBuilder } from "@koza-ai/core/effect/app-node-builder"
import { LayerNode } from "@koza-ai/core/effect/layer-node"

const originalEnv = {
  KOZA_AUTH_CONTENT: process.env.KOZA_AUTH_CONTENT,
  KOZA_EXPERIMENTAL_WORKSPACES: process.env.KOZA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.KOZA_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@koza.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.KOZA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=koza-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.KOZA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.KOZA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.KOZA_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=koza-test")
