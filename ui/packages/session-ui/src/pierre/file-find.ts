...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("koza-find")
  api.delete("koza-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("koza-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("koza-find", new Highlight(...rest))
    return true
  }
