import { ImagePreview } from "@koza-ai/ui/image-preview"
...
import { KeybindV2 } from "@koza-ai/ui/v2/keybind-v2"
import { TooltipV2 } from "@koza-ai/ui/v2/tooltip-v2"
import type { Prompt, ReferenceInfo } from "@koza-ai/sdk/v2/client"
import { createEffect, createMemo, on, Show, createSignal, onMount } from "solid-js"
...
import { PromptInputV2, type PromptInputV2Suggestion } from "@koza-ai/session-ui/v2/prompt-input"
import {
  createPromptInputV2Controller,
  createPromptInputV2State,
  type PromptInputV2Interaction,
} from "@koza-ai/session-ui/v2/prompt-input/interaction"
...
import { VoiceModeOverlay } from "@/components/voice/voice-mode-overlay"
import { voiceModeActive, setVoiceModeActive } from "@/components/voice/store"
...
export function PromptInputV2Composer(props: PromptInputV2ComposerProps) {
  ...
  const isProcessing = () => props.controller.model.loading || false
  ...
  const handleTranscript = (text: string) => {
    props.controller.identity.set(text, text.length)
    props.controller.submit()
    props.controller.identity.set("", 0)
  }

  const [activeKozaModel, setActiveKozaModel] = createSignal<{ provider: string, model: string } | null>(null)
  ...
  onMount(async () => {
    try {
      ...
      console.error("Failed to load koza model config:", e)
    }
  })
  ...
  return (
    <div class="flex flex-col gap-3">
      ...
      />
      <Show when={activeKozaModel()}>
        {(model) => (
          <div class="flex items-center gap-1.5 px-3 py-1 bg-surface-interactive-hover rounded-t-lg w-fit text-[11px] font-medium text-text-interactive-base -mb-3 z-10 border border-v2-border-border-base border-b-0">
            <span class="text-xs">⚡</span>
            <span class="capitalize">{model().provider}</span>
            <Show when={model().model && model().model !== "default"}>
              <span class="opacity-60">({model().model})</span>
            </Show>
          </div>
        )}
      </Show>
        ...
        modelControl={
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              class="flex items-center justify-center size-7 rounded hover:bg-surface-interactive-hover transition-colors"
              title="Voice Mode (Koza ile Konuş)"
            >
              <span class="text-text-interactive-base text-lg">🎙️</span>
            </button>
          </div>
        }
      />
