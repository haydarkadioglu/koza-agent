  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://koza.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://koza.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://koza.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://koza.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://koza.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://koza.example.com"),
      ),
    ).toEqual({
