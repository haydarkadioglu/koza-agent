...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      KOZA_CLI_NAME: `'${binary}'`,
      KOZA_MODELS_DEV: modelsData,
      KOZA_CHANNEL: `'${Script.channel}'`,
      KOZA_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/koza.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
