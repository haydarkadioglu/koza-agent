...
This is the living design and status document for `@koza-ai/codemode` and its existing V2 Koza adapter.
...
`packages/koza/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
