...
import { GoogleVertexPlugin } from "@koza-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.koza, (provider) => {
          provider.api = {
            ...
            url: "https://koza.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.koza))
