...
import { Permission } from "@koza-ai/schema/permission"
...
export class Service extends Context.Service<Service, Interface>()("@koza/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["koza.json", "koza.jsonc"]
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".koza")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".koza").toReversed()
    ...
    // global config, project files, then `.koza` files.
