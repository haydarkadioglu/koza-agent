...
import type { IntegrationOAuthMethodRegistration } from "@koza-ai/plugin/v2/effect/integration"
import { define } from "@koza-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@koza-ai/sdk/v2/types"
...
const defaultServer = "https://console.koza.ai"
const clientID = "koza-cli"
...
export const KozaPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "koza",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("KozaPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("koza")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Koza provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("koza", (integration) => {
        integration.name = "Koza"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "koza", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("koza")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("koza")
      ...
      const item = catalog.provider.get(ProviderV2.ID.koza)
      if (!item) return
      const hasKey = Boolean(process.env.KOZA_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("koza")),
