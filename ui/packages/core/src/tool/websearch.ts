...
import { ToolFailure } from "@koza-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@koza/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.KOZA_WEBSEARCH_PROVIDER === "exa" || process.env.KOZA_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.KOZA_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("KOZA_EXPERIMENTAL") || truthy("KOZA_ENABLE_EXA") || truthy("KOZA_EXPERIMENTAL_EXA"),
    enableParallel: truthy("KOZA_ENABLE_PARALLEL") || truthy("KOZA_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `koza/${InstallationVersion}`,
