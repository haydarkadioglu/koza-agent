...
import { createKoza } from "@koza-ai/sdk"
...
`)
  ...
  const koza = await createKoza({ port: 0 })
  ...
  try {
    ...
    const session = await koza.client.session.create()
    const result = await koza.client.session
      .prompt({
